/*
    ...
    Author: Kurt Kincaid
    Last Updated: 2025-05-26T22:38:39.571Z
    Version: 3.2.0
...
function shapiroWilk(v, alpha = 0.05) {
    ...
    // Calculate mi (expected values of the order statistics of a standard normal distribution)
    const mi = [];
    for (let i = 0; i < n; i++) {
        const p = ((i + 1) - 0.375) / (n + 0.25); // Calculate the percentile
        mi.push(jstat.normal.inv(p, 0, 1)); // Inverse CDF of the standard normal distribution
    }

    // Calculate m (sum of squares of mi)
    let m = mi.reduce((acc, val) => acc + Math.pow(val, 2), 0);

    // Calculate ai (coefficients for the test statistic)
    const u = 1 / Math.sqrt(n); // Calculate a scaling factor
    const ai = new Array(n).fill(0); // Initialize the coefficients array
    ...
    const W = Math.pow(sumA, 2) / Math.max(sumB, 0.0000001); // Avoid division by zero
